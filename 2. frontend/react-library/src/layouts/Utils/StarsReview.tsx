export const StarsReview: React.FC<{ rating: number, size: number }> = (props) => {
    ...
    if (rating !== undefined) {
        rating = Math.min(Math.max(rating, 0), 5); // Clamp rating between 0 and 5
        fullStars = Math.floor(rating);
        let decimalPart = rating - fullStars;
    
        halfStars = (decimalPart >= 0.25 && decimalPart <= 0.75) ? 1 : 0;
        fullStars += (decimalPart > 0.75) ? 1 : 0;
    } else {
        rating = 0;
    }
    ...
    return (
        <div>
            ...
            {/* Render a half star if the rating has a decimal part */}
            ...
            {/* Render an empty star for the remaining spots */}
